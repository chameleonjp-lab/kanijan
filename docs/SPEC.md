...
- 中央の 🦀 本体へ横・斜め上・斜め下から飛んでくる 🍣 または 🍤 に合わせてタップでジャンプさせ、高く積む。
- 旧足場案の絵文字は使用しない。
- 1プレイ最大60秒。60秒経過、バランスゲージ最大、リタイアで終了する。ぶつかっただけでは即終了せず、ゲージが100に達した時点で終了する。
- 操作はタップのみ。主操作は下部の大きな「🦀ジャンプ」ボタン。Canvas盤面タップでもジャンプできる。
- 状態は HOME / RULE / NAME / READY / PLAYING / ENDING / RESULT / ERROR に分ける。
- PLAYING 以外では当たり判定、スコア加算、残り時間減少を行わない。
- ENDING 以降はゲーム判定を止め、ENDINGで2秒間「終了！」「結果を集計中…」を表示してからRESULTへ進み、ランキング送信を1回だけ行う。
...
- 判定は横位置ではなく、食材の進行率と🦀中心までの距離で到達済みかを先に分け、未到達時だけ timeToHit を使う。
...
- 🍣基本速度は225。旧150の1.5倍で、上限320まで高さ補正を許可する。
- 🍤基本速度は370。旧185の2倍で、上限480まで高さ補正を許可する。
- 🍥基本速度は345。旧🍣基準速度150の2.3倍で、上限560まで高さ補正を許可する。50段以上で約20%の確率で出現し、50段未満では出現しない。
...
- 各食材は `resolved` 相当の処理済みフラグを持ち、1つの食材で複数回判定しない。
- `progress >= 1` または🦀中心距離が衝突半径以下の場合は、早すぎではなくぶつかった扱いにする。
- 減点後のスコアは0未満にしない。
- コンボボーナス: PERFECTまたはOK成功時に +10 × 現在コンボ数、1回最大300点。
- 到達ボーナス: 10段 +300点、20段 +700点、30段 +1000点。各初回のみ。
- 0点の通常終了は既存仕様どおり送信しない。ただしリタイアはプレイ回数計測のため0点でも送信する。
...
## 2026-06-19 連続被弾対策・終了画面・操作抑制
- HIT後は現在食材を `resolved` にし、`active=false` 相当で破棄してから0.55秒の復帰時間を置く。復帰中は次食材を出さず、当たり判定もしない。
- EARLY後は現在食材を `resolved` にし、0.32秒の復帰時間を置く。復帰中は次食材を出さず、古いジャンプ入力を持ち越さない。
- 復帰中のタップはスコア・ゲージ・高さ・コンボに反映せず、🦀を少し跳ねさせて「次に備えて！」を表示する。
- タイムアップ、バランス限界、リタイアの終了条件成立時は結果値を固定し、ENDING画面を2秒表示してからRESULTに移行する。ENDING中は食材移動、当たり判定、スコア加算、リタイア操作を反映しない。
- リタイアは0点でも `submit_score` を1回呼び、プレイ回数として計測できるようにする。通常の0点自然終了は既存仕様どおり未送信とする。
- 0〜49段は食材から🦀へのガイドラインを表示し、50段以上では🍣🍤🍥すべてのガイドラインを非表示にする。速度や判定値は変更しない。
- ゲーム画面、Canvas、ジャンプボタン、リタイアボタン、状態表示では `user-select`、`-webkit-touch-callout`、`touch-action`、`contextmenu` / `selectstart` 抑制で長押しメニューや文字選択を出にくくする。ホーム・結果画面のリンク操作は維持する。
